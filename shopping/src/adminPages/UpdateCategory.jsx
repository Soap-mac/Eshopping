...
function UpdateCategory() {
    ...
    useEffect(() => {
        const fetchCatDetails = async () => {
            try {
                const url = `${import.meta.env.VITE_API_URL}/getsinglecategory/${id}`;
                const response = await fetch(url, {
                    method: 'GET',
                    credentials: 'include'
                });
                const result = await response.json();
                console.log(result.existCategory);
                if (result.success) {
                    setCategoryName(result.existCategory.name);
                    // setPreviews((prev) => {
                    //     if (prev.includes(result.existCategory.image)) {
                    //         return prev;
                    //     }
                    //     return [...prev, result.existCategory.image];
                    // });
                    setCatImg(result.existCategory.image);
                }
            } catch (error) {
                console.log(error);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!categoryName && !categoryImage) {
            handleError('Please provide atleast one thing to change');
            return;
        }

        try {
            const url = `${import.meta.env.VITE_API_URL}/updateCategory/${id}`;
                                    ...
                                    <div className="!pt-4">
                                        ...
                                        >
                                            ...
                                            <div className="relative flex items-center justify-center gap-3 ">
                                                ...
                                                <span className="text-lg" type='Submit'>Save Changes</span>
